...
Before pre-processing the microarrays should be inspected visually for any
strong spatial biases. In \Biocpkg{PAA} this can be done for
\textit{ProtoArrays} using the function \Rfunction{plotArray()} which plots the
imported \textit{ProtoArray} data in the original arrangement mimicing the
original scan image. Thus, \Rfunction{plotArray()} is a visualization tool that
can be used to visualize \textit{ProtoArrays} for which the original scan image
is not available. Visual inspection of the spatial expression pattern can then
identify possible local tendencies and strong spatial biases. Moreover, the
array can be inspected at all stages of the pre-processing workflow in order to
check the impact of the particular methods that have been applied.
...
After pre-processing, the corrected data can be inspected via
\Rfunction{plotArray()} again. E.g., now the plot of the ProtoArray plotted
above shows that the revealed spatial bias is less obvious after pre-processing.
\begin{center}
